// PatchEtw.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
	...
	__in_ecount_opt(UserDataCount) PEVENT_DATA_DESCRIPTOR UserData)
{
	...
	_EtwEventWriteFull EtwEventWriteFull = (_EtwEventWriteFull)GetProcAddress(GetModuleHandle(L"ntdll.dll"), "EtwEventWriteFull");
...
BOOL PatchEtw()
{
	...
	*(DWORD64*)&uHook[2] = (DWORD64)MyEtwEventWrite;
	...
	return Patch(lpFuncAddress, uHook);
}

int main()
{
	if (PatchEtw()==0) {
		printf("[+] Patch EtwEventWrite Success\n");
	}
	else
	{
		printf("[-] Patch EtwEventWrite Failed\n");
	}
}
